...
class CrawlerComponent extends PositionComponent
  ...
  late final List<double>
      _segmentGridZ; // Current visual Z (pinned at 0 when falling)
  late final List<double>
      _segmentFallDistance; // Distance fallen along fall direction
  late final List<double> _segmentFallVelocity; // Speed along fall direction
  late final List<Vector2?>
      _segmentFallDirection; // Normalized fall direction vector
  ...
  void _updateSizeAndHitbox() {
    ...
    final relativeScale = perspective_scale_factor(
          headZ,
  ...
  ({
    ...
    return (
      grid_z: display_z,
      wobble_offset_y: wobble_offset_y,
      is_past_zero_time: is_past_zero_time
    );
  }
  ...
  void update(double dt) {
    ...
    for (int i = 0; i < numberOfSegments; i++) {
      ...
      if (_segmentFallDirection[i] != null) {
        level.map_grid_to_screen(grid_x, 0.0,
            out: _screenPos); // Base screen pos at Z=0
  ...
  void render(Canvas canvas) {
    ...
    for (int i = numberOfSegments - 1; i >= 0; i--) {
      ...
      // Calculate relative scale using perspective function
      final relativeScale = perspective_scale_factor(
            segmentGridZ, // Use the actual segment Z
            level.outer_scale_factor,
            level.deep_scale_factor,
          ) /
          level.outer_scale_factor;
      ...
      final int fillAlpha =
          (_baseFillAlpha * alphaMultiplier).round().clamp(0, 255);
      final int strokeAlpha =
          (_baseStrokeAlpha * alphaMultiplier).round().clamp(0, 255);

      final cf = hit_time > 0.0
          ? OnHit.hit_color
          : _segmentFillPaint.color.withAlpha(fillAlpha);
      final cs = hit_time > 0.0
          ? OnHit.hit_color
          : _segmentStrokePaint.color.withAlpha(strokeAlpha);
      _segmentFillPaint.color = cf;
      _segmentStrokePaint.color = cs;

      canvas.drawCircle(
          _relativePos.toOffset(), segmentVisualSize / 2, _segmentFillPaint);
      canvas.drawCircle(
          _relativePos.toOffset(), segmentVisualSize / 2, _segmentStrokePaint);
  ...
  @override
  double get grid_y =>
      0.0; // Crawlers move along the Z=0 plane (wobble is visual only)

  @override
  double get grid_z =>
      _segmentGridZ.isNotEmpty ? _segmentGridZ[0] : 0.0; // Use head segment Z
