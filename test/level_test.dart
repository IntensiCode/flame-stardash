...
import 'package:stardash/game/level/level_path.dart' show LevelPathType, LevelPath;
import 'package:test/test.dart';
import 'package:vector_math/vector_math_64.dart';

LevelGeometry _setupLevel(LevelPathType path_type) => LevelGeometry(path_type: path_type);
...
void expectVectorNotCloseTo(Vector2 actual, Vector2 expected, double tolerance) {
  expect(actual.x, isNot(closeTo(expected.x, tolerance)), reason: 'X component should not be close');
  expect(actual.y, isNot(closeTo(expected.y, tolerance)), reason: 'Y component should not be close');
}
  ...
  group('LevelLogic', () {
    ...
    group('precomputePathData', () {
      test('correctly initializes properties for V path', () {
        ...
        expect(level.path_definition, equals(LevelPath.definitions[LevelPathType.V]));
        ...
        expect(level.deep_scale_factor, closeTo(expectedBaseScale / 4.0, 0.001));
    ...
    group('mapGridToScreen', () {
      test('calculates center point identically for open path (V)', () {
        final expected = levelV.map_grid_to_screen(0.0, 0.5, clamp_and_wrap_x: true);
        final actual = levelV.map_grid_to_screen(0.0, 0.5, clamp_and_wrap_x: false);
        expectVectorCloseTo(actual, expected, epsilon);
      });

      test('calculates arbitrary point identically for closed path (FullPipe)', () {
        final expected = levelPipe.map_grid_to_screen(0.5, 0.5, clamp_and_wrap_x: true);
        final actual = levelPipe.map_grid_to_screen(0.5, 0.5, clamp_and_wrap_x: false);
        expectVectorCloseTo(actual, expected, epsilon);
      });

      // --- Tests for different gridZ values at gridX=0.0 ---
      test('calculates outer edge (gridZ=0.0) identically', () {
        final vExpected = levelV.map_grid_to_screen(0.0, 0.0, clamp_and_wrap_x: true);
        final vActual = levelV.map_grid_to_screen(0.0, 0.0, clamp_and_wrap_x: false);
        expectVectorCloseTo(vActual, vExpected, epsilon);

        final pipeExpected = levelPipe.map_grid_to_screen(0.0, 0.0, clamp_and_wrap_x: true);
        final pipeActual = levelPipe.map_grid_to_screen(0.0, 0.0, clamp_and_wrap_x: false);
        expectVectorCloseTo(pipeActual, pipeExpected, epsilon);
      });

      test('calculates deep edge (gridZ=1.0) identically', () {
        final vExpected = levelV.map_grid_to_screen(0.0, 1.0, clamp_and_wrap_x: true);
        final vActual = levelV.map_grid_to_screen(0.0, 1.0, clamp_and_wrap_x: false);
        expectVectorCloseTo(vActual, vExpected, epsilon);

        final pipeExpected = levelPipe.map_grid_to_screen(0.0, 1.0, clamp_and_wrap_x: true);
        final pipeActual = levelPipe.map_grid_to_screen(0.0, 1.0, clamp_and_wrap_x: false);
        expectVectorCloseTo(pipeActual, pipeExpected, epsilon);
      });

      test('calculates beyond outer edge (gridZ=-0.5) identically', () {
        final vExpected = levelV.map_grid_to_screen(0.0, -0.5, clamp_and_wrap_x: true);
        final vActual = levelV.map_grid_to_screen(0.0, -0.5, clamp_and_wrap_x: false);
        expectVectorCloseTo(vActual, vExpected, epsilon);

        final pipeExpected = levelPipe.map_grid_to_screen(0.0, -0.5, clamp_and_wrap_x: true);
        final pipeActual = levelPipe.map_grid_to_screen(0.0, -0.5, clamp_and_wrap_x: false);
        expectVectorCloseTo(pipeActual, pipeExpected, epsilon);
        // Note: The assertion allows gridZ outside [0, 1]
      });

      test('calculates beyond deep edge (gridZ=1.5) identically', () {
        final vExpected = levelV.map_grid_to_screen(0.0, 1.5, clamp_and_wrap_x: true);
        final vActual = levelV.map_grid_to_screen(0.0, 1.5, clamp_and_wrap_x: false);
        expectVectorCloseTo(vActual, vExpected, epsilon);

        final pipeExpected = levelPipe.map_grid_to_screen(0.0, 1.5, clamp_and_wrap_x: true);
        final pipeActual = levelPipe.map_grid_to_screen(0.0, 1.5, clamp_and_wrap_x: false);
        expectVectorCloseTo(pipeActual, pipeExpected, epsilon);
        // Note: The assertion allows gridZ outside [0, 1]
      });
    });
    ...
    group('mapGridToScreen on open path', () {
      test('clamps gridX < -1 when clampAndWrapX=true', () {
        ...
        final actual = levelV.map_grid_to_screen(-1.1, 0.0, clamp_and_wrap_x: true);
      ...
      test('extrapolates gridX < -1 when clampAndWrapX=false', () {
        final boundary = levelV.map_grid_to_screen(-1.0, 0.0);
        final actual = levelV.map_grid_to_screen(-1.1, 0.0, clamp_and_wrap_x: false);
        expect((actual.x - boundary.x).abs() > epsilon || (actual.y - boundary.y).abs() > epsilon, isTrue,
    ...
    group('mapGridToScreen on closed path', () {
      test('wraps gridX < -1 when clampAndWrapX=true', () {
        ...
        final actual = levelPipe.map_grid_to_screen(-1.1, 0.0, clamp_and_wrap_x: true);
        expectVectorCloseTo(actual, expected, epsilon);
      });
      ...
      test('wraps gridX far < -1 when clampAndWrapX=true', () {
        ...
        final expected = levelPipe.map_grid_to_screen(0.9, 0.0);
        final actual = levelPipe.map_grid_to_screen(-3.1, 0.0, clamp_and_wrap_x: true);
        expectVectorCloseTo(actual, expected, epsilon);
      });

      test('extrapolates gridX < -1 when clampAndWrapX=false', () {
        final boundary = levelPipe.map_grid_to_screen(-1.0, 0.0);
        final actual = levelPipe.map_grid_to_screen(-1.1, 0.0, clamp_and_wrap_x: false);
        expect((actual.x - boundary.x).abs() > epsilon || (actual.y - boundary.y).abs() > epsilon, isTrue,
            reason: "Extrapolated point should differ from boundary");
      });
    ...
    group('mapGridToScreen closed path properties', () {
      test('mapGridToScreen maps gridX=1.0 and gridX=-1.0 to same point for closed paths', () {
        ...
        final pointAtEndNoWrap = levelPipe.map_grid_to_screen(1.0, 0.0, clamp_and_wrap_x: false);
        final pointAtStartNoWrap = levelPipe.map_grid_to_screen(-1.0, 0.0, clamp_and_wrap_x: false);
    ...
    group('mapGridToScreen open path properties', () {
      test('mapGridToScreen extrapolation for open paths is currently bugged (jumps)', () {
        // Use levelV (open path) defined in setUpAll

        // Get points near ends (gridZ=0)
        final pointAtEnd = levelV.map_grid_to_screen(1.0, 0.0, clamp_and_wrap_x: false);
        final pointBeforeEnd = levelV.map_grid_to_screen(0.99, 0.0, clamp_and_wrap_x: false);
        final pointAtStart = levelV.map_grid_to_screen(-1.0, 0.0, clamp_and_wrap_x: false);
        final pointAfterStart = levelV.map_grid_to_screen(-0.99, 0.0, clamp_and_wrap_x: false);

        // Calculate expected *linear* extrapolated positions
        // Extrapolate by 0.1 (from 1.0 to 1.1, and -1.0 to -1.1)
        final endTangent = (pointAtEnd - pointBeforeEnd) * (0.1 / 0.01); // Scale delta by 10
        final expectedBeyondEnd = pointAtEnd + endTangent;

        final startTangent = (pointAtStart - pointAfterStart) * (0.1 / 0.01); // Scale delta by 10
        final expectedBeforeStart = pointAtStart + startTangent;

        // Get actual points calculated by the potentially bugged function
        final actualBeyondEnd = levelV.map_grid_to_screen(1.1, 0.0, clamp_and_wrap_x: false);
        final actualBeforeStart = levelV.map_grid_to_screen(-1.1, 0.0, clamp_and_wrap_x: false);
    ...
    group('getOrientationNormal', () {
      ...
      Vector2 calculateTangent(LevelGeometry level, double gridX) {
        const double tangentEpsilon = 0.001; // Small offset for tangent calculation
        ...
        return tangentVec.length2 < epsilon * epsilon
            ? Vector2(1, 0)
            : tangentVec.normalized(); // Return default tangent if length is near zero
      }
      ...
      test('returns normalized normal for HalfEight path at gridX=0 (concave center)', () {
      ...
      test('returns normalized normal for HalfEight path at gridX=0.2 (near concave)', () {
      ...
      test('returns normalized normal for HalfEight path at gridX=-0.8 (convex side)', () {
